...
type Tenant = { id: string; full_name: string; phone: string; email: string | null; identity_number: string | null; gender: string | null; date_of_birth: string | null; status: string; room_code: string | null; property_name: string | null };
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const body = section === "Nhà trọ"
      ...
      : section === "Phòng trọ"
        ...
        : { type: "tenant", id: editing?.id, fullName: form.get("fullName"), phone: form.get("phone"), email: form.get("email"), identityNumber: form.get("identityNumber"), gender: form.get("gender"), dateOfBirth: form.get("dateOfBirth"), roomId: form.get("roomId"), deposit: Number(form.get("deposit")), status: form.get("status") };
    const response = await fetch("/api/master-data", {
...
function DataForm({ section, editing, data, presetPropertyId, onClose, onSubmit }: { section: Section; editing: RecordItem | null; data: { properties: Property[]; rooms: Room[]; tenants: Tenant[] }; presetPropertyId?: string; onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) {
  ...
  return <div className="modal-backdrop" onMouseDown={onClose}><form className="modal master-form" onSubmit={onSubmit} onMouseDown={(e) => e.stopPropagation()}>
    ...
    {section === "Khách thuê" && <><div className="form-row"><label>Họ và tên<input name="fullName" defaultValue={tenant?.full_name || ""} required /></label><label>Số điện thoại<input name="phone" defaultValue={tenant?.phone || ""} required /></label></div><div className="form-row"><label>Email<input name="email" type="email" defaultValue={tenant?.email || ""} /></label><label>Số CCCD<input name="identityNumber" defaultValue={tenant?.identity_number || ""} /></label></div><div className="form-row"><label>Giới tính <small>(không bắt buộc)</small><select name="gender" defaultValue={tenant?.gender || ""}><option value="">Chưa cập nhật</option><option value="male">Nam</option><option value="female">Nữ</option><option value="other">Khác</option></select></label><label>Ngày sinh <small>(không bắt buộc)</small><input name="dateOfBirth" type="date" defaultValue={tenant?.date_of_birth?.slice(0, 10) || ""} /></label></div>{!editing && <><label>Xếp vào phòng<select name="roomId"><option value="">Chưa xếp phòng</option>{data.rooms.filter((r) => r.status === "vacant").map((r) => <option value={r.id} key={r.id}>{r.property_name} — {r.code}</option>)}</select></label><label>Tiền đặt cọc<input name="deposit" type="number" min="0" /></label></>}{editing && <label>Trạng thái<select name="status" defaultValue={tenant?.status}><option value="active">Đang hoạt động</option><option value="inactive">Ngừng hoạt động</option></select></label>}</>}
